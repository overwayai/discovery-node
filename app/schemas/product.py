...
class OfferBase(BaseModel):
    ...
    availability: str = Field(..., description="Availability status (e.g., 'InStock', 'OutOfStock')")
    inventory_level: Optional[int] = Field(None, description="Current inventory quantity")
    price_valid_until: Optional[datetime] = Field(None, description="Expiration date for the current price")
...
class ProductBase(BaseModel):
    ...
    product_group_id: Optional[UUID] = Field(None, description="ProductGroup this product belongs to")
    brand_id: UUID = Field(..., description="Brand this product belongs to")
    urn: str = Field(..., description="CMP product identifier (URN format)")
    variant_attributes: Dict[str, Any] = Field(default_factory=dict, description="Attributes that differentiate this variant")
    category_id: Optional[UUID] = Field(None, description="Category ID for this product")
    organization_id: UUID = Field(..., description="Organization this product belongs to")
...
class ProductCreate(ProductBase):
    ...
    additional_properties: Optional[List[PropertyValueBase]] = Field(None, description="Additional product properties")
...
class ProductForVector(BaseModel):
    ...
    product_group_id: Optional[str] = Field(None, description="Product group ID as string")
    variant_attrs: Dict[str, Any] = Field(default_factory=dict, description="Variant attributes")
    ...
    def from_product_with_relations(cls, product) -> "ProductForVector":
        ...
        return cls(
            ...
            product_group_id=str(product.product_group_id) if product.product_group_id else None,
...
class ProductSearchResponse(BaseModel):
    ...
    context: Dict[str, str] = Field(
        default={"schema": "https://schema.org", "cmp": "https://schema.commercemesh.ai/ns#"},
        alias="@context",
    ...
    cmp_totalResults: Optional[int] = Field(None, alias="cmp:totalResults", description="Total number of results")
    cmp_nodeVersion: Optional[str] = Field(None, alias="cmp:nodeVersion", description="Node version")
    datePublished: Optional[str] = Field(None, description="Publication date in ISO format")
