...
class FeedHandler:
        ...
        try:
            ...
            for item in items:
                ...
                if item_brand_id is None:
                    logger.warning(f"Skipping {item_type} '{item_data.get('name', 'unknown')}' - no brand information")
                    continue
                ...
                if item_type == "ProductGroup":
                    ...
                    brand = self.brand_service.get_by_name(item_data.get("brand", {}).get("name"))
                    if not brand:
                        logger.warning(f"Skipping ProductGroup '{item_data.get('name', 'unknown')}' - brand not found")
                        continue
                    organization_id = brand.organization_id
                    self.product_group_service.process_product_group(item_data, item_brand_id, organization_id)
                    ...
                    if category_name is None:
                        logger.warning(f"Skipping product '{item_data.get('name', 'unknown')}' - no category information")
                        continue
                    
                    # Process product with category information
                    self.product_service.process_product(item_data, item_brand_id, category_name)
    ...
    def _get_brand_id(self, item_data: Dict[str, Any]) -> Optional[UUID]:
        ...
        if not brand_name and item_data.get("@type") == "Product":
            ...
            if product_group_urn:
                ...
                if product_group and product_group.brand_id:
                    ...
                    logger.warning(f"Product {item_data.get('name', 'unknown')} references product group {product_group_urn} but no brand found")
                    return None
        ...
        logger.warning(f"Item {item_data.get('name', 'unknown')} has no brand information")
        return None
    ...
    def _get_category_name(self, item_data: Dict[str, Any]) -> Optional[str]:
        ...
        if not category_name and item_data.get("@type") == "Product":
            ...
            if product_group_urn:
                ...
                if product_group and product_group.category:
                    ...
                    logger.info(f"Product {item_data.get('name', 'unknown')} using category '{category_name}' from product group {product_group_urn}")
                else:
                    logger.warning(f"Product {item_data.get('name', 'unknown')} references product group {product_group_urn} but no category found")
                    return None
        ...
        logger.warning(f"Item {item_data.get('name', 'unknown')} has no category information")
        return None