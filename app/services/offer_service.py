...
class OfferService:
    ...
    def list_by_product(self, product_id: UUID, skip: int = 0, limit: int = 100) -> List[OfferInDB]:
        """List offers for a specific product"""
        offers = self.offer_repo.list_by_product(product_id, skip, limit)
        return [OfferInDB.model_validate(o) for o in offers]
    
    def list_by_seller(self, seller_id: UUID, skip: int = 0, limit: int = 100) -> List[OfferInDB]:
    ...
    def update_offer(self, offer_id: UUID, offer_data: OfferUpdate) -> Optional[OfferInDB]:
    ...
    def process_offer(self, offer_data: Dict[str, Any], product_id: UUID, seller_id: UUID) -> UUID:
        """
    ...
    def filter_offers(self, filters: Dict[str, Any], skip: int = 0, limit: int = 100) -> List[OfferInDB]:
