...
class BrandService:
    ...
    def get_by_name(self, name: str) -> Optional[BrandInDB]:
        ...
        brands = self.brand_repo.list(0, 1000)  # Get all brands (adjust limit as needed)
        for brand in brands:
            if brand.name == name:
                return BrandInDB.model_validate(brand)
        return None
    ...
    def list_by_organization(self, organization_id: UUID, skip: int = 0, limit: int = 100) -> List[BrandInDB]:
    ...
    def update_brand(self, brand_id: UUID, brand_data: BrandUpdate) -> Optional[BrandInDB]:
    ...
    def list_by_category(self, category_id: UUID, skip: int = 0, limit: int = 100) -> List[BrandInDB]:
