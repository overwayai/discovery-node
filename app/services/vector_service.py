...
class VectorService:
    ...
    def upsert_products(self, org_id: UUID):
        ...
        batch_num = 0
        while True:
            logger.debug(f"Fetching products with offset={offset}, batch_size={settings.PINECONE_BATCH_SIZE}, org_id={org_id}")
            products = self.product_repository.get_products_for_vector(
                offset, settings.PINECONE_BATCH_SIZE, org_id
            )
            if not products:
                logger.info(f"No more products to process. Exiting loop at offset={offset}.")
                break
            ...
            # Try dense index
            try:
                logger.info(f"Upserting {len(records)} records into dense index (batch {batch_num})")
                self.vector_repository.upsert_products_into_dense_index(records)
                logger.info(f"Dense index: successfully processed {len(records)} records (batch {batch_num})")
            except Exception as e:
            ...
            # Try sparse index  
            try:
                logger.info(f"Upserting {len(records)} records into sparse index (batch {batch_num})")
                self.vector_repository.upsert_products_into_sparse_index(records)
                logger.info(f"Sparse index: successfully processed {len(records)} records (batch {batch_num})")
            except Exception as e:
            ...
            if result.dense_index_success and result.sparse_index_success:
                ...
                logger.info(f"Batch {batch_num} processed successfully: {len(records)} records")
            else:
                logger.warning(f"Batch {batch_num} had errors. See result.errors for details.")
                
            offset += settings.PINECONE_BATCH_SIZE
            
        result.processing_time = time.time() - start_time
        logger.info(f"Finished upsert_products for org_id={org_id}. Total products: {result.total_products}, Successful: {result.successful_records}, Failed: {result.failed_records}, Time: {result.processing_time:.2f}s")
        if result.errors:
            logger.warning(f"Errors encountered during upsert_products: {result.errors}")
        return result
