...
class SearchService:
    ...
    def search_products(self, query: str, top_k: int = 20, alpha: float = 0.7, include_metadata: bool = True):
        ...
        try:
            ...
            logger.info(f"🔍 Querying Pinecone indices with Inference API (fetch_k={fetch_k})...")
            start_time = time.time()
            dense_results = self.vector_repository._search_dense_index(query, fetch_k, alpha, include_metadata)
            sparse_results = self.vector_repository._search_sparse_index(query, fetch_k, alpha, include_metadata)
    ...
    def _hits(self, resp):
        ...
        if hasattr(resp, 'result') and hasattr(resp.result, 'hits'):
            ...
            logger.debug(f"🔍 DEBUG: Using SearchRecordsResponse format, raw hits: {len(raw)}")
            ...
            if 'matches' in resp:
                ...
                raw = resp['result']['hits']
                logger.debug(f"🔍 DEBUG: Using 'result.hits' format, raw hits: {len(raw)}")
            else:
                logger.error(f"🔍 DEBUG: Unexpected response shape: {resp.keys() if isinstance(resp, dict) else 'Not a dict'}")
    ...
    def rrf_merge(self, dense_hits, sparse_hits, k=60, top_k=20):
        ...
        """
        logger.debug(f"🔍 DEBUG: RRF merge input - dense_hits: {len(dense_hits)}, sparse_hits: {len(sparse_hits)}")
        logger.debug(f"🔍 DEBUG: First dense hit: {dense_hits[0] if dense_hits else 'None'}")
        logger.debug(f"🔍 DEBUG: First sparse hit: {sparse_hits[0] if sparse_hits else 'None'}")
        ...
        search_results = []
        for hit_id, hit_data in sorted(fused.items(), key=lambda x: x[1]['score'], reverse=True)[:top_k]:
        ...
        logger.info(f"🔍 DEBUG: First merged result: {search_results[0] if search_results else 'None'}")
    ...
    def _enrich_with_product_data(self, search_results: List[SearchResult]) -> List[SearchResult]:
        ...
        try:
            ...
            logger.info(f"🔍 DEBUG: Extracted product IDs: {product_ids[:5]}...")  # Show first 5
            ...
            for product in products:
                if hasattr(product, 'offers') and product.offers:
                    ...
                    logger.info(f"🔍 DEBUG: Product {product.id} has {len(product.offers)} offers")
                else:
                    logger.info(f"🔍 DEBUG: Product {product.id} has no offers (offers attribute: {hasattr(product, 'offers')})")
            ...
            for product in products:
                ...
                if not media and product.product_group:
                    media = self._extract_media_from_jsonld(product.product_group.raw_data)
                ...
                prices = []
                logger.info(f"🔍 DEBUG: Product {product.id} has {len(product.offers) if product.offers else 0} offers")
                if product.offers:
                    # Use a set to track unique offers to avoid duplicates
                    seen_offers = set()
                    for offer in product.offers:
                        # Create a unique key for this offer
                        offer_key = (offer.price, offer.price_currency, offer.availability, offer.seller_id)
                        if offer_key not in seen_offers:
                            seen_offers.add(offer_key)
                            logger.info(f"🔍 DEBUG: Offer: price={offer.price}, currency={offer.price_currency}, availability={offer.availability}")
                            offer_data = {
                                'price': offer.price,
                            ...
                            if offer.price_valid_until:
                                offer_data['price_valid_until'] = offer.price_valid_until.isoformat()
                            if offer.inventory_level is not None:
                                offer_data['inventory_level'] = offer.inventory_level
                            if offer.shipping_speed_tier:
                                offer_data['shipping_speed_tier'] = offer.shipping_speed_tier
                            if offer.est_delivery_min_days is not None:
                                offer_data['est_delivery_min_days'] = offer.est_delivery_min_days
                            if offer.est_delivery_max_days is not None:
                                offer_data['est_delivery_max_days'] = offer.est_delivery_max_days
                            ...
                            if offer.return_window_days is not None:
                                offer_data['return_window_days'] = offer.return_window_days
                            if offer.gift_wrap is not None:
                                offer_data['gift_wrap'] = offer.gift_wrap
                            
                            prices.append(offer_data)
                        else:
                            logger.info(f"🔍 DEBUG: Skipping duplicate offer: price={offer.price}, currency={offer.price_currency}, availability={offer.availability}")
                else:
            ...
            logger.info(f"🔍 DEBUG: Product map keys: {list(product_map.keys())[:5]}...")  # Show first 5
            ...
            for result in search_results:
                ...
                if result_id in product_map:
                    ...
                    logger.info(f"🔍 DEBUG: Found product data for {result_id}: {data['name']}")
                    ...
                    if hasattr(result, 'product_name'):
                        ...
                        result.product_price = data['prices'][0]['price'] if data['prices'] else None
                        ...
                        logger.info(f"🔍 DEBUG: Set product_offers to: {result.product_offers}")
                    else:
                        # Dictionary
                        result['product_name'] = data['name']
                        result['product_brand'] = data['brand']
                        ...
                        result['product_url'] = data['url']
                        result['product_media'] = data['media']
                        logger.info(f"🔍 DEBUG: Set product_offers to: {result.get('product_offers')}")
                else:
                ...
                if hasattr(result, 'metadata') and result.metadata:
                    ...
                    if hasattr(result, 'product_category') and not result.product_category:
    ...
    def _extract_media_from_jsonld(self, jsonld: Dict[str, Any]) -> List[Dict[str, Any]]:
