...
from app.schemas.product import ProductCreate, ProductUpdate, ProductInDB, PropertyValueBase
...
class ProductService:
    ...
    def get_by_sku(self, sku: str, brand_id: Optional[UUID] = None) -> Optional[ProductInDB]:
    ...
    def list_by_product_group(self, product_group_id: UUID, skip: int = 0, limit: int = 100) -> List[ProductInDB]:
        """List products by product group"""
        products = self.product_repo.list_by_product_group(product_group_id, skip, limit)
        return [ProductInDB.model_validate(p) for p in products]
    
    def search_products(self, query: str, skip: int = 0, limit: int = 100) -> List[ProductInDB]:
    ...
    def update_product(self, product_id: UUID, product_data: ProductUpdate) -> Optional[ProductInDB]:
        """Update an existing product"""
        # Business logic validation
        if product_data.urn:
            existing = self.product_repo.get_by_urn(product_data.urn)
            if existing and existing.id != product_id:
                logger.warning(f"Another product with URN {product_data.urn} already exists")
    ...
    def process_product(self, product_data: Dict[str, Any], brand_id: UUID, category_name: str) -> UUID:
        """
        ...
        if "additionalProperty" in product_data:
            ...
            for prop in props:
                if "@type" in prop and "name" in prop and "value" in prop:
                    name = prop["name"]
                    value = prop["value"]
                    additional_properties.append(PropertyValueBase(name=name, value=value))
    ...
    def filter_products(self, filters: Dict[str, Any], skip: int = 0, limit: int = 100) -> List[ProductInDB]:
