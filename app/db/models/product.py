...
class Product(Base):
    ...
    urn = Column(String, unique=True, nullable=False, comment="CMP-specific product identifier (URN format)")
    name = Column(String, nullable=False, index=True)
    url = Column(String)
    sku = Column(String, index=True, comment="Stock keeping unit")
    description = Column(Text)
    product_group_id = Column(UUID(as_uuid=True), ForeignKey("product_groups.id", ondelete="CASCADE"), nullable=True)
    ...
    variant_attributes = Column(JSONB, default={}, comment="Attributes that differentiate this variant")
