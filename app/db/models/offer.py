...
class Offer(Base):
    ...
    seller_id = Column(UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False, 
                      comment="Organization offering the product")
    
    # Pricing information
    price = Column(Float, nullable=False, comment="Current price of the product")
    price_currency = Column(String(3), nullable=False, comment="Currency code (e.g., 'USD')")
    price_valid_until = Column(TIMESTAMP(timezone=True), comment="Expiration date for the current price")
    
    # Availability information
    availability = Column(String, nullable=False, comment="Availability status (e.g., 'InStock', 'OutOfStock')")
    ...
    shipping_speed_tier = Column(String, comment="Shipping speed tier (e.g., 'Standard', 'Express')")
    est_delivery_min_days = Column(Integer, comment="Minimum expected delivery time in days")
    est_delivery_max_days = Column(Integer, comment="Maximum expected delivery time in days")
    ...
    # Additional flags
    gift_wrap = Column(Boolean, default=False, comment="Whether gift wrapping is available")
