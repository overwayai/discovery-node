...
class OfferRepository:
    ...
    def list_by_product(self, product_id: UUID, skip: int = 0, limit: int = 100) -> List[Offer]:
        """List offers for a specific product"""
        return self.db_session.query(Offer).filter(
            Offer.product_id == product_id
        ).offset(skip).limit(limit).all()
    
    def list_by_seller(self, seller_id: UUID, skip: int = 0, limit: int = 100) -> List[Offer]:
    ...
    def list_by_filter(self, filters: Dict[str, Any], skip: int = 0, limit: int = 100) -> List[Offer]:
        """
        ...
        if "shipping_destination" in filters and filters["shipping_destination"] is not None:
            query = query.filter(Offer.shipping_destination == filters["shipping_destination"])
