...
class ProductGroupRepository:
    ...
    def get_by_id(self, product_group_id: UUID) -> Optional[ProductGroup]:
        ...
        return self.db_session.query(ProductGroup).filter(ProductGroup.id == product_group_id).first()
    
    def get_by_urn(self, urn: str) -> Optional[ProductGroup]:
        """Get product group by CMP URN"""
        return self.db_session.query(ProductGroup).filter(ProductGroup.urn == urn).first()
    ...
    def get_by_product_group_id(self, product_group_id: str) -> Optional[ProductGroup]:
        ...
        return self.db_session.query(ProductGroup).filter(
    ...
    def list_by_brand(self, brand_id: UUID, skip: int = 0, limit: int = 100) -> List[ProductGroup]:
        """List product groups by brand"""
        return self.db_session.query(ProductGroup).filter(
            ProductGroup.brand_id == brand_id
        ).offset(skip).limit(limit).all()
    
    def list_by_category(self, category: str, skip: int = 0, limit: int = 100) -> List[ProductGroup]:
        """List product groups by category string"""
        return self.db_session.query(ProductGroup).filter(
            ProductGroup.category == category
        ).offset(skip).limit(limit).all()
    
    def list_by_organization(self, organization_id: UUID, skip: int = 0, limit: int = 100) -> List[ProductGroup]:
    ...
    def update(self, product_group_id: UUID, product_group_data: ProductGroupUpdate) -> Optional[ProductGroup]:
        """Update an existing product group"""
        db_product_group = self.get_by_id(product_group_id)
        if not db_product_group:
            return None
        product_group_data_dict = product_group_data.model_dump(exclude_unset=True, exclude={"category"})
    ...
    def list_by_category_id(self, category_id: UUID, skip: int = 0, limit: int = 100) -> List[ProductGroup]:
        """List product groups by category ID (using category_id column)"""
        return self.db_session.query(ProductGroup)\
